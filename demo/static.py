...
def video_loop_mp_hands(source, process_data_func):
    ...
    with mp_hands.Hands(static_image_mode=True, model_complexity=1,max_num_hands=1,min_detection_confidence=0.5,min_tracking_confidence=0.5) as hands:
        while video.isOpened():
            ...
            cv2.imshow("Asl fingerspelling demo", cv2.resize(image, (int(SCALE_FACTOR*image.shape[1]), int(SCALE_FACTOR*image.shape[0]))))
...
if __name__ == "__main__":
    ...
    base_model = tf.saved_model.load(os.path.join("trained_models", "static_fingerspelling_demo_model"))
